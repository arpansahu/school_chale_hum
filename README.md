...
[![Rancher](https://img.shields.io/badge/Rancher-0075A8?style=for-the-badge&logo=rancher&logoColor=white)](https://rancher.com/)s
...
server {
    ...
    location / {
         proxy_pass              http://{ip_of_home_server}:8013;
         proxy_set_header        Host $host;
         proxy_set_header        X-Forwarded-Proto $scheme;
  ...
  2. Nginx for Kubernetes Deployment 
    ...
    ```bash
        server {
            ...
            location / {
                proxy_pass http://<CLUSTER_IP_ADDRESS>:32013;
                proxy_set_header Host $host;
                proxy_set_header X-Forwarded-Proto $scheme;
...
pipeline {
    ...
    environment {
        ...
        REPOSITORY = "library/school_chale_hum"
...
pipeline {
    ...
    environment {
        ...
        NGINX_CONF = "/etc/nginx/sites-available/school-chale-hum"
        ENV_PROJECT_NAME = "school_chale_hum"
        DOCKER_PORT = "8013"
...
Make sure to use Pipeline project and name it whatever you want I have named it as per school_chale_hum
...
Make sure to use Pipeline project and name it whatever you want I have named it as school_chale_hum_build
...
If image is pushed successfully, email is sent to notify and then another Jenkins Pipeline school_chale_hum_build is called.
...
* Add a .env file to you project using following command (This step is no more required stage('Dependencies'))
    ...
    ```bash
    sudo vi  /var/lib/jenkins/workspace/school_chale_hum/.env
    ```
